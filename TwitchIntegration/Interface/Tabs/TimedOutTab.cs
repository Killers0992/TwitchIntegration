...
{
    public partial class TimedOutTab : PoisonUserControl, IActionRemovable
    {
        ...
        {
            MainClass.Instance.Config.Events.OnUserTimedout.GlobalDelay = new TimeSpan((int)globalH.Value, (int)globalM.Value, (int)globalS.Value);
        }

        public void Init()
        {
            _isUpdating = true;
            RandomExecution = MainClass.Instance.Config.Events.OnUserTimedout.ExecuteRandomAction;

            globalH.Value = MainClass.Instance.Config.Events.OnUserTimedout.GlobalDelay.Hours;
            globalM.Value = MainClass.Instance.Config.Events.OnUserTimedout.GlobalDelay.Minutes;
            globalS.Value = MainClass.Instance.Config.Events.OnUserTimedout.GlobalDelay.Seconds;
        ...
        public void LoadActions()
        {
            ...
            OscActions = MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions;
        }
        ...
        public void AddNewAction(int id, string actionName, double duration, string value, string defaultValue, bool scroll = false)
        {
            if (id == -1)
            {
                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions.Add(new OscOutAction()
                {
                ...
                id = MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions.Count - 1;
            ...
            {
                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].ActionName = item.actionName.Text;
                MainClass.Instance.SaveConfig();
            };
            item.executionDuration.TextChanged += (o, e) =>
            {
                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].ExecutionDuration = Convert.ToDouble(item.executionDuration.Value);
                MainClass.Instance.SaveConfig();
            };
            item.oscValue.TextChanged += (o, e) =>
            {
                var parsed = item.oscValue.Text.ConvertStringToObject();
                var currentValue = MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].Value;

                if (currentValue == parsed)
                    return;

                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].Value = parsed;
                MainClass.Instance.SaveConfig();
            };
            item.defaultValue.TextChanged += (o, e) =>
            {
                var parsed = item.defaultValue.Text.ConvertStringToObject();
                var currentValue = MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].DefaultValue;

                if (currentValue == parsed)
                    return;

                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions[item.ID].DefaultValue = parsed;
                MainClass.Instance.SaveConfig();
            };
            item.removeAction.Click += (o, e) =>
            {
                ...
                MainClass.Instance.Config.Events.OnUserTimedout.OscOutActions.RemoveAt(item.ID);
                MainClass.Instance.SaveConfig();
        ...
        {
            MainClass.Instance.Config.Events.OnUserTimedout.ExecuteRandomAction = randomAction.Switched;
            MainClass.Instance.SaveConfig();
        }
